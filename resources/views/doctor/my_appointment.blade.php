...
<tr style="background-color:black;" >
    ...
    <td>
      <form action="{{ route('admin.cancel_appointment', ['id' => $appoints->id]) }}" method="POST" style="display: inline;">
        @csrf
        @method('PUT')
        <button type="submit" class="btn btn-danger" onclick="return confirm('Are you sure to cancel this?')">Cancel</button>
      </form>
      {{-- <a class="btn btn-danger" onclick="return confirm('Are you sure to delete this?')"  href="{{route('cancel_appointment', ['id' $appoints->id])}}">Cancel --}}
    </td>
</tr>
